...
function register($no)
    ...
    $data = '{"email":"'.$email.'@gmail.com","name":"'.$nama.'","phone":"+'.$no.'","signed_up_country":"US"}';
...
function reff($token)
    ...
    $claim = request("/wallet/qr-code?pho ne_number=%2B".$no, $token, null);
    if ($claim['data'] == null)
        {
		return true;
        }
